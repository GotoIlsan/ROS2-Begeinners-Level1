...
class TurtleControllerNode(Node):
    def __init__(self):
        super().__init__("tuetle_controller")
        self.declare_parameter("catch_closest_turtle_first", True)

        self.turtle_to_catch = None
        self.catch_closet_turtle_first = self.get_parameter("catch_closest_turtle_first").value
    ...
    def callback_alive_turtles(self, msg):
        if len(msg.turtles) > 0:
            if self.catch_closet_turtle_first:
                closest_turtle = None
    ...
    def call_catch_turtle_server(self, turtle_name):
        ...
        while not client.wait_for_service(1.0):
            self.get_logger().info("Waiting catch call Server...")
    ...
    def callback_call_catch_turtle(self, future, turtle_name):
        try:
            ...
            if not response.success:
                self.get_logger().error(f"Turtle {turtle_name} colud not be caught")
        except Exception as e:
            self.get_logger().error(f"Service call for catch_turtle Failed : {e}")
